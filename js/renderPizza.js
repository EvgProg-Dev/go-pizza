...
const createCard = (data) => {
    ...
    card.innerHTML = `
            <picture>
                ...
                <img class="card__img" src="${data.images[0]}" alt="${
        data.name.ru
    }">
            </picture>
...
export const renderPizzas = async (toppings) => {
    const pizzas = await getData(
        `https://wonderful-puzzled-parrot.glitch.me/api/products${
        ...
        modalController({
            ...
            async cbOpen(btnOpen) {
                const pizza = await getData(
                    `https://wonderful-puzzled-parrot.glitch.me/api/products/${btnOpen.dataset.id}`
                );
